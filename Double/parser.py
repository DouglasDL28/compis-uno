...
class Parser():
    ...
    def Expr(self):
        while self.la.name in {"(","number","-","decnumber"}:
            self.Stat()
            self.expect(";")
    ...
    def Factor(self, result):
        ...
        if self.la.name in {"-"}:
            self.expect("-")
            sign=-1;
        if self.la.name in {"number","decnumber"}:
            result = self.Number(result)
        elif self.la.name in {"("}:
            self.expect("(")
            result = self.Expression(result)
            self.expect(")")
        else: self.syn_error({'(', 'number', 'decnumber'})
        result*=sign;
        return result
    ...
    def Number(self, result):
        if self.la.name in {"number"}:
            self.expect("number")
        elif self.la.name in {"decnumber"}:
            self.expect("decnumber")
        else: self.syn_error({'number', 'decnumber'})
        result=float(self.t.lexeme);
        return result
